...
jQuery('.bycwooodtlite_deactivation_confirm').click(function(){
	...
	if(!jQuery("input[name='bycwooodtlite_deactivation_cause']:checked").val()){
		...
		alert("Please help us to improve the plugin by providing a reason of deactivation!");
			...
			jQuery("form[name='deactivate_bycwooodtlite']").css("text-align","center");
			
			jQuery("form[name='deactivate_bycwooodtlite']").html("<h1>Thank you for feedback<h1> <h2>Deactivating...<h2><img src='../wp-content/plugins/byconsole-woo-order-delivery-time/images/Ripple_200x200.gif' />");

			var wooodtlite_deactivation_call=jQuery.ajax({

				method:"POST",

				url:"https://api.byconsole.com/public/api/sites?p="+window.location.href, 
